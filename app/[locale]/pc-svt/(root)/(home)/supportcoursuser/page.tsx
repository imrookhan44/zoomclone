...
const UserPage = () => {
  ...
  const [filteredFiles, setFilteredFiles] = useState<{ name: string, url: string }[]>([]); // State to store filtered files

  const t = useTranslations(); // Hook to get translations

  useEffect(() => {
    const fetchFiles = async () => {
      const storageRef = ref(storage, 'supportcourspcsvt/');
      ...
      <div className="container mx-auto p-6 bg-gradient-to-r from-[#508C9B] to-[#87CEEB] rounded-lg shadow-2xl">
        ...
        {currentFile ? (
          ...
          <>
            <input 
              type="text" 
              placeholder={t('searchPlaceholdercourse')}
              value={searchQuery}
